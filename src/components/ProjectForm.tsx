...
interface ProjectFormProps {
  project?: FormProject;
  onSubmit: (projectData: Partial<FormProject>) => Promise<{ success: boolean; error?: string }>;
  onCancel: () => void;
  isLoading?: boolean;
}
...
const PROJECT_CATEGORIES: { value: string; label: string }[] = [
  ...
  { value: 'Library/Package', label: 'Library/Package' },
...
export default function ProjectForm({ project, onSubmit, onCancel, isLoading = false }: ProjectFormProps) {
  const [formData, setFormData] = useState({
    ...
    startDate: project?.startDate || project?.created_at?.split('T')[0] || new Date().toISOString().split('T')[0],
    endDate: project?.endDate || '',
    githubUrl: project?.githubUrl || project?.github_repo || '',
