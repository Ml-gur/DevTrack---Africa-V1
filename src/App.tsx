...
import { offlineDatabase as localDatabase } from "./utils/offline-database-wrapper";
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      {isProductionMode() && (
        <div className="bg-green-600 text-white p-1 text-center text-xs">
          🚀 DevTrack Africa - Production Platform (Supabase)
        </div>
      )}
      ...
      {isDevelopmentMode() && (
        <div className="bg-blue-600 text-white p-1 text-center text-xs">
          🔧 DevTrack Africa - Development Environment (Supabase)
        </div>
      )}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700">
                      Track your projects, manage tasks with Kanban boards, and monitor your productivity. All data is securely stored in Supabase with real-time sync.
                    </p>
                  </div>
                </div>
...
function App() {
  ...
  useEffect(() => {
    // Register service worker on mount (only in production or HTTPS)
    const registerSW = async () => {
      // Only register service worker in supported environments
      if (!('serviceWorker' in navigator)) {
        console.log('ℹ️ Service Worker not supported in this environment');
        return;
      }

      // Skip registration in Figma preview or other unsupported environments
      const hostname = window.location.hostname;
      ...
      if (!isLocalhost && !isHTTPS) {
        console.log('ℹ️ Service Worker requires HTTPS (except on localhost)');
        return;
      }
      ...
      try {
        ...
        // Gracefully handle service worker registration failures
        // This is expected in some environments (Figma preview, HTTP, etc.)
